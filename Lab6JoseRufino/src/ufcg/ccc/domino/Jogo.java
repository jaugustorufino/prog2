...
public class Jogo {
	...
	public void rodada() throws EstrategiaInvalidaException, JogadaInvalidaException {
		...
		if (jogador1.getNumPecas() == 0) {
			...
			if (jogadaJ1.getTipo() == TipoJogada.NA_DIREITA) {
				if (mesa.getNumNaEsquerda() == jogadaJ1.getPeca().getNumDireito()) {
					this.pontuacaoVencedor += 3;
					this.tipoVitoria = TipoVitoria.LA_E_LO;
					// Cruzada
					if (jogadaJ1.getPeca().getNumDireito() == jogadaJ1.getPeca().getNumEsquerdo()) {
						this.pontuacaoVencedor += 3;
			...
			if (jogadaJ1.getTipo() == TipoJogada.NA_ESQUERDA) {
				if (mesa.getNumNaDireita() == jogadaJ1.getPeca().getNumEsquerdo()) {
					this.pontuacaoVencedor += 3;
					this.tipoVitoria = TipoVitoria.LA_E_LO;
					// Cruzada
					if (jogadaJ1.getPeca().getNumDireito() == jogadaJ1.getPeca().getNumEsquerdo()) {
			...
			// Vitória Normal
			this.pontuacaoVencedor++;
			this.tipoVitoria = TipoVitoria.NORMAL;
			return;
		}
		...
		if (jogador2.getNumPecas() == 0) {
			...
			if (jogadaJ2.getTipo() == TipoJogada.NA_DIREITA) {

				// La e Lo
				if (mesa.getNumNaEsquerda() == jogadaJ2.getPeca().getNumDireito()) {
					this.pontuacaoVencedor += 3;
					this.tipoVitoria = TipoVitoria.LA_E_LO;
					// Cruzada
					if (jogadaJ2.getPeca().getNumDireito() == jogadaJ2.getPeca().getNumEsquerdo()) {
						this.pontuacaoVencedor += 3;
						this.tipoVitoria = TipoVitoria.CRUZADA;
					}
					return;
			...
			if (jogadaJ2.getTipo() == TipoJogada.NA_ESQUERDA) {

				// La e Lo
				if (mesa.getNumNaDireita() == jogadaJ2.getPeca().getNumEsquerdo()) {
					this.pontuacaoVencedor += 3;
					this.tipoVitoria = TipoVitoria.LA_E_LO;
					// Cruzada
					if (jogadaJ2.getPeca().getNumDireito() == jogadaJ2.getPeca().getNumEsquerdo()) {
		...
		if (jogadaJ1.getTipo() == TipoJogada.PASSA && jogadaJ2.getTipo() == TipoJogada.PASSA) {
			// Checa numero de peças de cada jogador (Menos peças ganha)
			if (jogador1.getNumPecas() < jogador2.getNumPecas()) {
				// J1 venceu por ter menos peças
				this.finalizado = true;
				this.vencedor = this.jogador1.getNome();
				this.pontuacaoVencedor++;
				return;
			...
			// Checa soma de pontos de cada jogador (Menos pontos ganha)
			
			if (jogador1.getSomaPontos() < jogador2.getSomaPontos()) {
				// J1 venceu pela soma de pontos ser menor
				this.finalizado = true;
				this.vencedor = this.jogador1.getNome();
				this.pontuacaoVencedor++;
				return;
			} else if (jogador2.getSomaPontos() < jogador1.getSomaPontos()) {
				// J2 venceu pela soma de pontos ser menor
				this.finalizado = true;
				this.vencedor = this.jogador2.getNome();
				this.pontuacaoVencedor++;
				return;
			...
			// Se nenhum dos critérios de desempate foi utilizado, é empate
			this.finalizado = true;
			this.vencedor = null;
		}
	}
	...
	public HistoricoDeJogo jogaJogoCompleto() throws EstrategiaInvalidaException, JogadaInvalidaException {
		...
		if (this.isResultadoEmpate()) {
			...
			jogado.setVencedor(getVencedor(), this.pontuacaoVencedor);
			jogado.setTipoVitoria(this.tipoVitoria);
		}
